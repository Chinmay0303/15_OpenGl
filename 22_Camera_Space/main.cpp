...
// Vector3f CameraPos(0.0f,0.0f,-2.0f);
Vector3f CameraPos(3.0f,0.0f,-2.0f);

// UVN axis system
Vector3f U(1.0f,0.0f,0.0f); // right direction to N (X)
Vector3f V(0.0f,1.0f,0.0f); // up direction (Y)
Vector3f N(0.0f,0.0f,1.0f); // direction where camera is looking (Z)
...
void RenderSceneCB(){
    ...
    Matrix4f View = CameraMatrix();

    Matrix4f WVP = P * View * World;
